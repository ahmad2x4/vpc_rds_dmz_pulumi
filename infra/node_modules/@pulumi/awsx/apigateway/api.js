...
class API extends pulumi.ComponentResource {
    constructor(name, args, opts = {}) {
        ...
        if (args.swaggerString) {
            ...
            const result = createSwaggerSpec(this, name, args.routes || [], pulumi.output(args.additionalRoutes || []), args.gatewayResponses, args.requestValidator, args.apiKeySource, args.staticRoutesBucket);
            ...
            throw new pulumi.ResourceError("API must specify either [swaggerString] or as least one of the [route] options.", opts.parent);
        }
        const stageName = args.stageName || "stage";
        const restApiArgs = args.restApiArgs || {};
        // Create the API Gateway Rest API, using a swagger spec.
        this.restAPI = new aws.apigateway.RestApi(name, Object.assign(Object.assign({}, args.restApiArgs), { name: utils_1.ifUndefined(restApiArgs.name, title), binaryMediaTypes: utils_1.ifUndefined(restApiArgs.binaryMediaTypes, ["*/*"]), body: swaggerString }), { parent: this });
        // Create a deployment of the Rest API.
        this.deployment = new aws.apigateway.Deployment(name, Object.assign(Object.assign({}, args.deploymentArgs), { restApi: this.restAPI, 
            ...
            variables: {
                version: swaggerString.apply(s => utils_1.sha1hash(s)),
            } }), { parent: this });
        ...
        this.stage = new aws.apigateway.Stage(name, Object.assign(Object.assign({}, args.stageArgs), { restApi: this.restAPI, deployment: this.deployment, stageName: stageName }), { parent: this, dependsOn: permissions });
...
exports.API = API;
function createLambdaPermissions(api, name, swaggerLambdas) {
    const permissions = [];
...
function createSwaggerSpec(api, name, routes, additionalRoutes, gatewayResponses, requestValidator, apikeySource, bucketOrArgs) {
...
function checkRoute(api, route, propName) {
    if (route[propName] === undefined) {
        throw new pulumi.ResourceError(`Route missing required [${propName}] property`, api);
    }
}
function addEventHandlerRouteToSwaggerSpec(api, name, swagger, swaggerLambdas, route, apiAuthorizers) {
    checkRoute(api, route, "eventHandler");
    checkRoute(api, route, "method");
    const method = swaggerMethod(route.method);
    const lambda = aws.lambda.createFunctionFromEventHandler(name + utils_1.sha1hash(method + ":" + route.path), route.eventHandler, { parent: api });
    ...
    function createSwaggerOperationForLambda() {
        const region = utils_1.getRegion(api);
...
function addBasePathOptionsToSwagger(api, swagger, swaggerOperation, route, apiAuthorizers) {
...
function addAuthorizersToSwagger(api, swagger, authorizers, apiAuthorizers) {
    const authRecords = [];
    swagger["securityDefinitions"] = swagger["securityDefinitions"] || {};
...
function getLambdaAuthorizer(api, authorizerName, authorizer) {
...
function addStaticRouteToSwaggerSpec(api, name, swagger, route, bucket, apiAuthorizers) {
...
function addIntegrationRouteToSwaggerSpec(api, name, swagger, route, apiAuthorizers) {
    checkRoute(api, route, "target");
    const target = isIntegrationRouteTargetProvider(route.target)
        ? pulumi.output(route.target.target(name + utils_1.sha1hash(route.path), api))
...
function addRawDataRouteToSwaggerSpec(api, name, swagger, route) {
