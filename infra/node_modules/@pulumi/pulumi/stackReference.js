...
const invoke = require("./runtime/invoke");
...
class StackReference extends resource_1.CustomResource {
    ...
    readOutputSync(callerName, outputName, required) {
...
For more details see: https://www.pulumi.com/docs/troubleshooting/#stackreference-sync`);
        ...
        return [utils_1.promiseResult(out.promise()), utils_1.promiseResult(out.isSecret)];
    }
    readOutputsSync(callerName) {
        ...
        if (this.stackReferenceName instanceof Promise) {
            ...
            console.log(`Call made to StackReference.${callerName} with a StackReference with a Promise name. This is now deprecated and may cause the program to hang.
For more details see: https://www.pulumi.com/docs/troubleshooting/#stackreference-sync`);
            stackName = utils_1.promiseResult(this.stackReferenceName);
        }
        else if (output_1.Output.isInstance(this.stackReferenceName)) {
            console.log(`Call made to StackReference.${callerName} with a StackReference with an Output name. This is now deprecated and may cause the program to hang.
For more details see: https://www.pulumi.com/docs/troubleshooting/#stackreference-sync`);
            stackName = utils_1.promiseResult(this.stackReferenceName.promise());
        }
        else {
            stackName = this.stackReferenceName;
        }
        try {
            const res = invoke.invokeSync("pulumi:pulumi:readStackOutputs", { name: stackName });
