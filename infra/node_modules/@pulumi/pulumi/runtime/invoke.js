...
function invoke(tok, props, opts = {}) {
    ...
    const config = new config_1.Config("pulumi");
 ...
 * @internal
 */
function invokeSync(tok, props, opts = {}) {
    const syncInvokes = settings_1.tryGetSyncInvokes();
    if (!syncInvokes) {
    ...
    return invokeSyncWorker(tok, props, opts, syncInvokes);
}
...
function invokeFallbackToAsync(tok, props, opts) {
    return utils.promiseResult(invokeAsync(tok, props, opts));
}
exports.invokeFallbackToAsync = invokeFallbackToAsync;
...
function invokeSyncWorker(tok, props, opts, syncInvokes) {
    ...
    function getProviderRefSync() {
        ...
        if (provider.__registrationId === undefined) {
            ...
            console.log(`Synchronous call made to "${tok}" with an unregistered provider. This is now deprecated and may cause the program to hang.
For more details see: https://www.pulumi.com/docs/troubleshooting/#synchronous-call`);
            utils.promiseResult(resource_1.ProviderResource.register(provider));
        }
...
function serializePropertiesSync(prop) {
    if (prop === undefined ||
        prop === null ||
        typeof prop === "boolean" ||
        typeof prop === "number" ||
