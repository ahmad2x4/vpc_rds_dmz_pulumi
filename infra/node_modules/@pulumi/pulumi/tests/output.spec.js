...
describe("output", () => {
    ...
    describe("isKnown", () => {
        ...
        it("is unknown if the value is or contains unknowns", util_1.asyncTest(() => __awaiter(void 0, void 0, void 0, function* () {
            ...
            assert.equal(yield o1.isKnown, false);
            assert.equal(yield o2.isKnown, false);
            assert.equal(yield o3.isKnown, false);
        })));
        it("is unknown if the result after apply is unknown or contains unknowns", util_1.asyncTest(() => __awaiter(void 0, void 0, void 0, function* () {
            ...
            assert.equal(yield r1.isKnown, false);
            assert.equal(yield r2.isKnown, false);
            assert.equal(yield r3.isKnown, false);
            assert.equal(yield r4.isKnown, false);
            assert.equal(yield r5.isKnown, false);
            assert.equal(yield r6.isKnown, false);
        })));
    });
    describe("concat", () => {
    ...
    describe("lifted operations", () => {
        ...
        it("lifts properties from values with nested unknowns", util_1.asyncTest(() => __awaiter(void 0, void 0, void 0, function* () {
            ...
            assert.equal(yield output1.isKnown, false);
            const result1 = output1.foo;
            assert.equal(yield result1.isKnown, true);
            assert.equal(yield result1.promise(/*withUnknowns*/ true), "foo");
            const result2 = output1.bar;
            assert.equal(yield result2.isKnown, false);
            assert.equal(yield result2.promise(/*withUnknowns*/ true), output_1.unknown);
            const result3 = output1.baz;
            assert.equal(yield result3.isKnown, false);
            assert.equal(yield result3.promise(/*withUnknowns*/ true), output_1.unknown);
            const result4 = output1.qux;
            assert.equal(yield result4.isKnown, false);
            assert.equal(yield result4.promise(/*withUnknowns*/ true), output_1.unknown);
            const result5 = output1.baz.qux;
            assert.equal(yield result5.isKnown, false);
            assert.equal(yield result5.promise(/*withUnknowns*/ true), output_1.unknown);
            const output2 = output_1.output(["foo", output_1.unknown, mockOutput(false, undefined)]);
            assert.equal(yield output2.isKnown, false);
            const result6 = output2[0];
            assert.equal(yield result6.isKnown, true);
            assert.equal(yield result6.promise(/*withUnknowns*/ true), "foo");
            const result7 = output2[1];
            assert.equal(yield result7.isKnown, false);
            assert.equal(yield result7.promise(/*withUnknowns*/ true), output_1.unknown);
            const result8 = output2[2];
            assert.equal(yield result8.isKnown, false);
            assert.equal(yield result8.promise(/*withUnknowns*/ true), output_1.unknown);
            const output3 = output_1.all([output_1.unknown, mockOutput(false, undefined), output_1.output(["foo", output_1.unknown])]);
            assert.equal(yield output3.isKnown, false);
            const result9 = output3[0];
            assert.equal(yield result9.isKnown, false);
            assert.equal(yield result9.promise(/*withUnknowns*/ true), output_1.unknown);
            const result10 = output3[1];
            assert.equal(yield result10.isKnown, false);
            assert.equal(yield result10.promise(/*withUnknowns*/ true), output_1.unknown);
            const result11 = output3[2];
            assert.equal(yield result11.isKnown, false);
            const result12 = result11[0];
            assert.equal(yield result12.isKnown, true);
            assert.equal(yield result12.promise(/*withUnknowns*/ true), "foo");
            const result13 = result11[1];
            assert.equal(yield result13.isKnown, false);
            assert.equal(yield result13.promise(/*withUnknowns*/ true), output_1.unknown);
            ...
            assert.equal(yield output4.isKnown, false);
            const result14 = output4.foo;
            assert.equal(yield result14.isKnown, false);
            assert.equal(yield result14.promise(/*withUnknowns*/ true), output_1.unknown);
            const result15 = output4.bar;
            assert.equal(yield result15.isKnown, false);
            assert.equal(yield result15.promise(/*withUnknowns*/ true), output_1.unknown);
            const result16 = output4.baz;
            assert.equal(yield result16.isKnown, false);
            const result17 = result16.foo;
            assert.equal(yield result17.isKnown, true);
            assert.equal(yield result17.promise(/*withUnknowns*/ true), "foo");
            const result18 = result16.qux;
            assert.equal(yield result18.isKnown, false);
            assert.equal(yield result18.promise(/*withUnknowns*/ true), output_1.unknown);
        })));
    });
});
